# similar to eval.py - added argument: target - specify what COCO object to create mask for
...
def parse_args(argv=None):
    ...
    parser.add_argument('--target', default=None, type=str,
                        help='A target input that will be masked.')
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    def get_color(j, on_gpu=None):
        ...
        global args
        offset = 255
        if args.target is not None and args.target != cfg.dataset.class_names[classes[j]]:
            offset = 0
        ...
        if on_gpu is not None and color_idx in color_cache[on_gpu]:
            ...
            if not undo_transform:
                ...
                color = (color[2]*0+offset, color[1]*0+offset, color[0]*0+offset)
